...
class GradeForm(FlaskForm):
    ...
    term = RadioField('Wybierz semestr:', choices=[('term1', 'Semestr pierwszy'),('term2', 'Semestr drugi')], validators=[DataRequired()] )
    category = SelectField('Wybierz kategorie:', choices=[('answer', 'Odpowiedź'),('quiz', 'Quiz'),('test', 'Sprawdzian')], validators=[DataRequired()] )
    grade = SelectField('Wybierz ocene:', choices=[
        (1,'1'),
        (2,'2'),
        (3,'3'),
        (4,'4'),
...
def calculateAvg(grades, out_type=0):
    ...
    for subject, terms in grades.items():
        ...
        if out_type == 1:
            ...
            all_iter += 1
            output = round(all_sum/all_iter,2)
...
def addSubject():
    ...
    if addSubject.validate_on_submit():
        with open('data/grades.json', encoding='utf-8') as gradesFile:
            ...
            gradesFile.close()
            subject = addSubject.subject.data.lower()
            grades[subject]={
                "term1": {
                    "answer": [],
                    "quiz": [],
...
def addGrade():
    ...
    with open('data/grades.json', encoding='utf-8') as gradesFile:
        grades = json.load(gradesFile)
        subjects = [(subject, subject.capitalize()) for subject in grades.keys()]
        print(subjects)
        addGrade.subject.choices = subjects
        gradesFile.close()
    ...
    if addGrade.validate_on_submit():
        ...
        grades[subject][term][category].append(int(addGrade.grade.data))
        with open('data/grades.json','w', encoding='utf-8') as gradesFile:
            json.dump(grades, gradesFile)
            gradesFile.close()
            redirect('/dashboard')

    return render_template('add-grade.html', title='Dodaj ocene', userLogin=session.get('userLogin'), firstName=session.get('firstName'), date=date, addGrade=addGrade)
...
def dashboard():
    ...
    air_quality_src = requests.get("https://api.gios.gov.pl/pjp-api/rest/aqindex/getIndex/400")
    air_quality_krakow = json.loads(air_quality_src.content)
    return render_template('dashboard.html', title='Dashboard', userLogin=session.get('userLogin'), firstName=session.get('firstName'), date=date, grades=grades,calculateAvg = calculateAvg, pogoda=pogoda_krakow, air_quality=air_quality_krakow)
